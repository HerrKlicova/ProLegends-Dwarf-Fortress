...
const UI = (() => {
  ...
  /* Un evento contado como una frase. Si el narrador no supo contarlo, al
     menos se enseña el nombre del suceso y sus datos: nunca se inventa nada. */
  function sucesoTexto(ev) {
    return ev.frase || (tipoLegible(ev.tipo) + (ev.detalles ? ' · ' + detallesTexto(ev.detalles) : ''));
  }

  function suceso(ev) {
    return el('span', { text: sucesoTexto(ev) });
  }
