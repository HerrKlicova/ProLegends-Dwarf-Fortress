/* Crónicas narradas por IA. Nunca se llama a la API sin confirmacion expresa. */
const Cronicas = (() => {
  const el = UI.el;
    ...
    UI.poner(panel, el('div', {}, [
      el('div', { class: 'ficha-cabecera' }, [
        ...
        el('div', { class: 'tipo', text:
          ...
          'Cada crónica se guarda: pedirla otra vez no cuesta nada.' }),
      ]),
      ...
      UI.bloque('Crónica de un rango de años',
        el('div', { class: 'chips' }, [
      ...
      UI.bloque('Otras crónicas',
        el('p', { class: 'nota', text:
          'La crónica de una figura concreta se pide desde su ficha, en la pestanya de figuras históricas. ' +
          'La de tu fortaleza, desde la pestanya "Mi fortaleza".' })),

      guardadas.cronicas.length ? UI.bloque(`Crónicas ya guardadas (${guardadas.cronicas.length})`,
        UI.tabla(['Ámbito', 'Título', 'Modelo', 'Generada'], guardadas.cronicas.map((c) => [
          c.scope_key,
          el('span', { class: 'enlace', text: c.title || '(sin título)',
  ...
  async function pedir(ambito, etiqueta) {
    ...
    UI.poner(salida, el('p', { class: 'vacio', text: 'Escribiendo la crónica...' }));
    try {
      const r = await API.generarCron({ export_id: App.exportId(), ambito, regenerar: false });
      volcar(r, ambito);
      ...
      el('p', {}, [el('button', { class: 'boton', text: 'Volver a generarla (consume API)',
        onclick: async () => {
          const ok = await UI.confirmar('Regenerar la crónica',
            [el('p', { text: 'Se hará una llamada nueva a la API y se sustituira el texto guardado.' })],
            'Regenerar');
          if (!ok) return;
