/* Panel de MI fortaleza: deteccion, resumen, novedades entre exports y avisos. */
const Fortaleza = (() => {
  const el = UI.el;
  ...
  function pintar(d) {
    ...
    return el('div', {}, [
      ...
      el('div', { class: 'tarjetas' }, [
        tarjeta(r.fundacion === null || r.fundacion === undefined ? '?' : r.fundacion, 'Anyo de fundacion'),
        tarjeta((r.habitantes || {}).vivos ?? 0, 'Figuras vivas vinculadas'),
        tarjeta((r.habitantes || {}).muertos ?? 0, 'Figuras fallecidas'),
        tarjeta(((r.artefactos || {}).creados_aqui || []).length, 'Artefactos creados aqui'),
      ...
      n ? novedades(n, d) : el('p', { class: 'nota', text:
        'Solo hay un export de este mundo. Cuando importes otro mas reciente aparecera aqui que ha cambiado.' }),
      ...
      UI.bloque('Ficha', UI.datos([
        ...
        ['Civilizacion', r.civilizacion ? el('span', { class: 'enlace', text: r.civilizacion.name,
          onclick: () => Figuras.verEntidad(r.civilizacion.entity_id) }) : null],
        ['Censo de la civilizacion', (r.censo_civilizacion || []).length
          ? r.censo_civilizacion.map((c) => `${c.race}: ${c.count}`).join(', ') : null],
      ])),

      (r.artefactos && r.artefactos.creados_aqui.length) ? UI.bloque('Artefactos creados aqui',
        UI.tabla(['Anyo', 'Artefacto', 'Autor'], r.artefactos.creados_aqui.map((x) => [
      ...
      (r.caravanas || []).length ? UI.bloque('Caravanas recibidas',
        UI.tabla(['Anyo', 'Suceso', 'Datos del archivo'], r.caravanas.map((c) => [
      ...
      (r.ataques || []).length ? UI.bloque('Ataques registrados',
        UI.tabla(['Anyo', 'Suceso', 'Datos del archivo'], r.ataques.map((c) => [
          String(UI.anyo(c.anyo)), UI.tipoLegible(c.tipo),
          el('span', { class: 'nota', text: UI.detallesTexto(c.detalles) }),
        ]))) : null,
      ...
      (r.habitantes && r.habitantes.lista_vivos.length) ? UI.bloque(
        ...
        UI.tabla(['Nombre', 'Raza', 'Vinculo'], r.habitantes.lista_vivos.map((h) => [
      ...
      (r.habitantes && r.habitantes.lista_muertos.length) ? UI.bloque(
        ...
        UI.tabla(['Nombre', 'Raza', 'Anyo'], r.habitantes.lista_muertos.map((h) => [
          el('span', { class: 'enlace', text: h.name || '?', onclick: () => App.irAFigura(h.hf_id) }),
          h.race || '—', String(UI.anyo(h.death_year)),
        ]))) : null,

      el('div', { class: 'bloque' }, [
        el('button', { class: 'boton', text: 'Generar cronica de mi fortaleza',
          onclick: () => Cronicas.pedir({ tipo: 'fortaleza', site_id: d.seleccion.site_id },
            `Cronica de ${r.sitio ? r.sitio.nombre : 'mi fortaleza'}`) }),
  ...
  function novedades(n, d) {
    ...
    const bloques = secciones.filter(([, lista]) => lista && lista.length).map(([titulo, lista, clase]) =>
      UI.bloque(`${titulo} (${lista.length})`, ...lista.slice(0, 60).map((ev) =>
        el('div', { class: clase }, [
          el('strong', { text: `Anyo ${UI.anyo(ev.anyo)} · ${UI.tipoLegible(ev.tipo)}` }),
    ...
    if (n.artefactos_nuevos.length) {
      bloques.push(UI.bloque(`Artefactos nuevos (${n.artefactos_nuevos.length})`,
        UI.tabla(['Artefacto', 'Objeto', 'Donde'], n.artefactos_nuevos.slice(0, 80).map((a) => [
          a.name || '?', a.item || '—',
    ...
    if (n.muertes_nuevas.length) {
      bloques.push(UI.bloque(`Han muerto desde el export anterior (${n.muertes_nuevas.length})`,
        UI.tabla(['Nombre', 'Raza', 'Anyo'], n.muertes_nuevas.slice(0, 100).map((m) => [
  ...
  function avisos(a) {
    ...
    for (const g of (a.guerras || [])) {
      filas.push(el('div', { class: 'alerta' }, [
        ...
        document.createTextNode(`${g.name || 'sin nombre'} — ${g.atacante || '?'} contra ${g.defensor || '?'} (desde el anyo ${UI.anyo(g.start_year)})`),
      ]));
    }
  ...
  function sinFortaleza(d) {
    ...
    return el('div', {}, [
      ...
      el('p', { class: 'nota', text: d.seleccion.ambiguo
        ...
        : 'No se ha podido deducir cual es tu fortaleza. Importa un export mas reciente del mismo mundo, o elige el sitio a mano.' }),
      candidatos.length ? UI.tabla(['Sitio', 'Tipo', 'Fundado', 'Por que', ''],
        candidatos.map((c) => [
          c.nombre || `sitio ${c.site_id}`, c.tipo || '—', String(UI.anyo(c.fundado)),
          el('span', { class: 'nota', text: c.razones.join('; ') }),
          el('button', { class: 'boton pequeno', text: 'Es esta',
  ...
  async function elegir(d) {
    ...
    const ok = await UI.confirmar('Elegir mi fortaleza', [
      el('p', { class: 'nota', text: 'La eleccion se guarda para este mundo y manda sobre la deteccion automatica.' }),
      buscador, lista,
      el('p', {}, [el('button', { class: 'boton pequeno', text: 'Olvidar la eleccion guardada',
